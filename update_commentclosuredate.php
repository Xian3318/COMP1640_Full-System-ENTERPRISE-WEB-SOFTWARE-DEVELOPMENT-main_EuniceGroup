...
<nav class="navbar navbar-inverse">
  <div class="container-fluid">
    <div class="navbar-header">
	 ...
	 <div id="Edit Details" class="tabcontent">
          ...
          <?php
			...
			{
				$edit_cat = $_GET['ideaID'];
				
			}

            $query = "SELECT *  FROM  idea WHERE ideaID=$edit_cat";
			$select_posts = mysqli_query($conn,$query);


            while($row = mysqli_fetch_assoc($select_posts)) 
			{
				$categoryName = $row['ideaTitle'];
				$categoryTime = $row['commentClosureDate'];
			...
			$query = "UPDATE idea SET commentClosureDate = '$categoryTime' WHERE ideaID = '$edit_cat' ";
			
			if($conn->query($query)==TRUE)
			{
				echo"<script>alert('Updated Comment Successfully');</script>";
	...
	<div class="container">
	<div class="row">
	<h3>Edit Comment Closure Date</h3>
    <form action="" method="post" enctype="multipart/form-data">
	
	<div class="form-group">
		<label for="categoryName">Idea Title</label>
		<input value="<?php echo $categoryName; ?>" class="form-control" name="categoryName" disabled>
	</div>
	
	<div class="form-group">
		<label for="categoryTime">Comment Closure Date</label>
